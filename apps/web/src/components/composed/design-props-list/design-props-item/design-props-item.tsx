...
type PropItem = FigmaPropsData[number]['props'][number];

export interface DesignPropsItemProps {
  /**
 ...
 */
export const DesignPropsItem = ({ className, property, defaultExpanded = false }: DesignPropsItemProps) => {
    ...
    <div role="button" tabIndex={0} onKeyDown={handleClick} className={classes.container} onClick={handleClick}>
      ...
      {isExpanded && (
        <div className="flex flex-col gap-2">
          ...
          {/* FIGMA TYPE */}
          <div className="flex items-center gap-2">
            <div className="text-sm text-neutral-50 font-semibold">Figma Type:</div>
            <code className={classes.tag(TagVariant.default)}>{property?.figmaType}</code>
          </div>
