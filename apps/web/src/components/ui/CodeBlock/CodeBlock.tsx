...
const lineFocusedStyles = tv({
  base: ['[&_code:has(.focused)_.line]:blur-[2px]', '[&_code:has(.focused)_.line.focused]:blur-none'],
});

const wordHighlightStyles = tv({
  base: ['[&_.highlighted-word]:bg-blue-50', 'dark:[&_.highlighted-word]:!bg-blue-500/10'],
});
...
export const CodeBlock = ({
  ...
  return (
    <CodeBlockContext.Provider value={{ value, onValueChange, data }}>
      <div className={codeBlockContainer({ className })} {...props} />
...
export const CodeBlockFiles = ({ className, children, ...props }: CodeBlockFilesProps) => {
  ...
  return (
    <div className={codeBlockFiles({ className })} {...props}>
      {data.map(children)}
    </div>
  );
};
...
export const CodeBlockCopyButton = ({
  ...
  return (
    <Button
      className={codeBlockCopyButton({ className })}
      onClick={copyToClipboard}
      size={ButtonSize.sm}
